...
uint64_t GemFS_Dir_to_Index(char* directory){
    uint64_t len = calculate_string_length(directory);
}
...
bool GemFS_FBB_GetBlock(enum GemFS_DriveIDs DriveID, uint8_t Partition, uint64_t Block){
    ...
    FLOPPY_Read_LBA(DriveID, GemFS_BlockToLBA(DriveID, Partition, t_block+3), (uint64_t)FBB_Block, BS);
...
void GemFS_FBB_SetBlock(enum GemFS_DriveIDs DriveID, uint8_t Partition, uint64_t Block){
    ...
    FLOPPY_Write_LBA(DriveID, GemFS_BlockToLBA(DriveID, Partition, t_block+3), (uint64_t)FBB_Block, BS);
}
...
uint64_t GemFS_GetEntryBlock(enum GemFS_DriveIDs DriveID, uint8_t Partition, uint64_t Index){
    ...
    //return entry.Next_Index_Start;
    return GemFS_ReadEntry(DriveID, Partition, Index-1).Next_Index_Start;
}
...
struct GemFS_Entry GemFS_ReadEntry(enum GemFS_DriveIDs DriveID, uint8_t Partition, uint64_t Index){
    ...
    if(Index == 0){
        #ifdef GEMFS_SANITY
            SetTextColor(LCYAN, BLACK);
            printf("Reading entry %i from block: %x\n", Index, Block);
            SetTextColor(WHITE, BLACK);
        #endif
...
void GemFS_WriteEntry(enum GemFS_DriveIDs DriveID, uint8_t Partition, uint64_t Index, struct GemFS_Entry entry){
    ...
    if(Index == 0){
        ...
        #ifdef GEMFS_SANITY
            SetTextColor(LCYAN, BLACK);
            printf("Writing entry %i to block: %x (Main)\n", Index, Block);
        #endif
    ...
    #ifdef GEMFS_SANITY
        SetTextColor(LCYAN, BLACK);
        printf("Writing entry %i to block: %x\n", Index, Block);
        SetTextColor(WHITE, BLACK);
    #endif
    memset(&Entry_Buffer, 0, 512);
        ...
        if(partition.Type == 0xC8){
            ...
            if(correct){
                ...
                GemFS_DumpEntryData(main_entry.Entry);
                    ...
                    printf("Drive %i Partition %i is labeled as GemFS, but not formatted.\n", DriveID, i);
                    SetTextColor(WHITE, BLACK);
                #endif
...
void GemFS_CreateEntry(enum GemFS_DriveIDs DriveID, uint8_t Partititon, uint64_t Index, uint8_t flags){
    ...
    #ifdef GEMFS_SANITY
        SetTextColor(LCYAN, BLACK);
        printf("Creating entry %i at block %x, LastEntry=%i\n", Index, free_block, Index-1);
        SetTextColor(WHITE, BLACK);
    #endif
