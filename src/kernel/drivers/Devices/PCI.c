...
PCI_Device* FLOPPY_Controller = NULL;
...
void ScanBusses(){
    ...
    for(uint8_t bus = 0; bus < 8; bus++){
        for(uint8_t slot = 0; slot < 32; slot++){
            ...
            }else if(curHeader.ClassCode == PCI_CLASS_STR && curHeader.SubClass == 0x02 && curHeader.ProgIF == 0x00){
                FLOPPY_Controller = &Devices[deviceRun];
            }
            deviceRun++;
        }
    }
